            ...
            if (document.isObject())
            {
                ...
                if (obj.contains("time") && obj.contains("states"))
                {
                    ...
                    for (int i = 0; i < states.size(); i++)
                    {
                        ...
                        aircraft->m_onSurfaceValid = true;
                        if (!state[9].isNull()) {
                            aircraft->setGroundspeed((int)std::round(Units::metresPerSecondToKnots(state[9].toDouble())), aircraft->m_rxTime, m_settings);
                        }
                        if (!state[10].isNull()) {
                            aircraft->setTrack((float)state[10].toDouble(), aircraft->m_rxTime);
                        }
                        if (!state[11].isNull()) {
                            aircraft->setVerticalRate((int)std::round(Units::metresPerSecondToFeetPerMin(state[11].toDouble())), aircraft->m_rxTime, m_settings);
                        }
                        if (!state[14].isNull())
                        {
