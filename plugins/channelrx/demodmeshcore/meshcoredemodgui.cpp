...
MeshcoreDemodGUI* MeshcoreDemodGUI::create(PluginAPI* pluginAPI, DeviceUISet *deviceUISet, BasebandSampleSink *rxChannel)
{
	MeshcoreDemodGUI* gui = new MeshcoreDemodGUI(pluginAPI, deviceUISet, rxChannel);
	return gui;
}

void MeshcoreDemodGUI::destroy()
{
	delete this;
}

void MeshcoreDemodGUI::resetToDefaults()
{
...
QByteArray MeshcoreDemodGUI::serialize() const
{
    return m_settings.serialize();
}

bool MeshcoreDemodGUI::deserialize(const QByteArray& data)
{
    resetLoRaStatus();
...
bool MeshcoreDemodGUI::handleMessage(const Message& message)
{
    if (DSPSignalNotification::match(message))
    {
        ...
        qDebug() << "MeshcoreDemodGUI::handleMessage: DSPSignalNotification: m_basebandSampleRate: " << basebandSampleRate;
        ...
        if (m_remoteTcpReconnectAutoApplyPending)
        {
            ...
            qInfo() << "MeshcoreDemodGUI::handleMessage: DSPSignalNotification after RemoteTCP reconnect - reapplying Meshcore profile";
            QMetaObject::invokeMethod(this, &MeshcoreDemodGUI::applyMeshcoreProfileFromSelection, Qt::QueuedConnection);
        }

        return true;
    }
    else if (MeshcoreDemodMsg::MsgReportDecodeBytes::match(message))
    {
        // Populates the upper unstructured view including raw bytes in hex
        const MeshcoreDemodMsg::MsgReportDecodeBytes& msg = (MeshcoreDemodMsg::MsgReportDecodeBytes&) message;
        handleMeshAutoLockObservation(msg);
        showLoRaMessage(message);
        return true;
    }
    else if (MeshcoreDemodMsg::MsgReportDecodeString::match(message))
    {
        // Populates the lower structured tree view with decoded fields
        showTextMessage(message);
        return true;
    }
    else if (MeshcoreDemod::MsgConfigureMeshcoreDemod::match(message))
    {
        qDebug("MeshcoreDemodGUI::handleMessage: NFMDemod::MsgConfigureMeshcoreDemod");
        const MeshcoreDemod::MsgConfigureMeshcoreDemod& cfg = (MeshcoreDemod::MsgConfigureMeshcoreDemod&) message;
...
void MeshcoreDemodGUI::handleInputMessages()
{
    Message* message;
...
void MeshcoreDemodGUI::channelMarkerChangedByCursor()
{
...
void MeshcoreDemodGUI::on_deltaFrequency_changed(qint64 value)
{
...
void MeshcoreDemodGUI::channelMarkerHighlightedByCursor()
{
    setHighlighted(m_channelMarker.getHighlighted());
}

void MeshcoreDemodGUI::on_BW_valueChanged(int value)
{
    auto& s = focusedSettings();
    if (value < 0) {
        s.m_bandwidthIndex = 0;
    } else if (value < MeshcoreDemodSettings::nbBandwidths) {
        s.m_bandwidthIndex = value;
    } else {
        s.m_bandwidthIndex = MeshcoreDemodSettings::nbBandwidths - 1;
    }

	int thisBW = MeshcoreDemodSettings::bandwidths[s.m_bandwidthIndex];
...
void MeshcoreDemodGUI::on_Spread_valueChanged(int value)
{
...
void MeshcoreDemodGUI::on_deBits_valueChanged(int value)
{
...
void MeshcoreDemodGUI::on_preambleChirps_valueChanged(int value)
{
...
void MeshcoreDemodGUI::on_mute_toggled(bool checked)
{
    m_settings.m_decodeActive = !checked;
    applySettings();
}

void MeshcoreDemodGUI::on_clear_clicked(bool checked)
{
    (void) checked;
    clearPipelineViews();
...
void MeshcoreDemodGUI::on_eomSquelch_valueChanged(int value)
{
...
void MeshcoreDemodGUI::on_messageLength_valueChanged(int value)
{
...
void MeshcoreDemodGUI::on_udpSend_stateChanged(int state)
{
    m_settings.m_sendViaUDP = (state == Qt::Checked);
    applySettings();
}

void MeshcoreDemodGUI::on_udpSendJson_stateChanged(int state)
{
    m_settings.m_sendJsonViaUDP = (state == Qt::Checked);
    applySettings();
}

void MeshcoreDemodGUI::on_udpAddress_editingFinished()
{
    m_settings.m_udpAddress = ui->udpAddress->text();
    applySettings();
}

void MeshcoreDemodGUI::on_udpPort_editingFinished()
{
    bool ok;
...
void MeshcoreDemodGUI::on_invertRamps_stateChanged(int state)
{
    focusedSettings().m_invertRamps = (state == Qt::Checked);
    applyFocusedPipelineSettings();
}

void MeshcoreDemodGUI::on_meshRegion_currentIndexChanged(int index)
{
    (void) index;
...
void MeshcoreDemodGUI::on_meshPreset_currentIndexChanged(int index)
{
    (void) index;
    if (m_meshControlsUpdating) {
        return;
...
void MeshcoreDemodGUI::on_meshChannel_currentIndexChanged(int index)
{
    (void) index;
    if (m_meshControlsUpdating) {
        return;
...
void MeshcoreDemodGUI::on_meshApply_clicked(bool checked)
{
    (void) checked;
...
void MeshcoreDemodGUI::on_meshKeys_clicked(bool checked)
{
    (void) checked;
    editMeshcoreKeys();
}

void MeshcoreDemodGUI::on_meshAutoSampleRate_toggled(bool checked)
{
    if (m_meshControlsUpdating) {
        return;
    }

    m_settings.m_meshcoreAutoSampleRate = checked;
    applySettings();
...
void MeshcoreDemodGUI::on_meshAutoLock_clicked(bool checked)
{
    if (checked) {
        startMeshAutoLock();
    } else {
...
void MeshcoreDemodGUI::startMeshAutoLock()
{
    if (m_meshAutoLockActive) {
        return;
    }

        const int bandwidthHz = MeshcoreDemodSettings::bandwidths[m_settings.m_bandwidthIndex];
...
void MeshcoreDemodGUI::stopMeshAutoLock(bool keepBestCandidate)
{
...
void MeshcoreDemodGUI::applyMeshAutoLockCandidate(const MeshAutoLockCandidate& candidate, bool applySettingsNow)
{
...
void MeshcoreDemodGUI::handleMeshAutoLockObservation(const MeshcoreDemodMsg::MsgReportDecodeBytes& msg)
{
    if (!m_meshAutoLockActive) {
        return;
    }
    ...
    if (headerParityStatus == (int) MeshcoreDemodSettings::ParityOK) {
        score += 8.0;
    } else if (headerParityStatus == (int) MeshcoreDemodSettings::ParityCorrected) {
        score += 5.0;
    } else if (headerParityStatus == (int) MeshcoreDemodSettings::ParityError) {
        score -= 7.0;
    }
    ...
    if (!earlyEOM)
    {
        if (payloadParityStatus == (int) MeshcoreDemodSettings::ParityOK) {
            score += 6.0;
        } else if (payloadParityStatus == (int) MeshcoreDemodSettings::ParityCorrected) {
            score += 3.0;
        } else if (payloadParityStatus == (int) MeshcoreDemodSettings::ParityError) {
            score -= 4.0;
        }
    ...
    candidate.headerParityOkOrFixCount += (headerParityStatus == (int) MeshcoreDemodSettings::ParityOK
        || headerParityStatus == (int) MeshcoreDemodSettings::ParityCorrected) ? 1 : 0;
...
void MeshcoreDemodGUI::handleMeshAutoLockSourceObservation()
{
    if (!m_meshAutoLockActive) {
        return;
    }
    ...
    MeshAutoLockCandidate& candidate = m_meshAutoLockCandidates[m_meshAutoLockCandidateIndex];
    const double totalPower = std::max(1e-12, m_meshcoreDemod->getTotalPower());
    const double noisePower = std::max(1e-12, m_meshcoreDemod->getCurrentNoiseLevel());
    ...
    const bool demodActive = m_meshcoreDemod->getDemodActive();
...
void MeshcoreDemodGUI::advanceMeshAutoLock()
{
    if (!m_meshAutoLockActive) {
        return;
    }
...
void MeshcoreDemodGUI::editMeshcoreKeys()
{
    MeshcoreKeysDialog dialog(this);
    dialog.setKeySpecList(m_settings.m_meshcoreKeySpecList);

    if (dialog.exec() != QDialog::Accepted) {
        return;
    }

    m_settings.m_meshcoreKeySpecList = dialog.getKeySpecList();

    const bool hasCustomKeys = !m_settings.m_meshcoreKeySpecList.isEmpty();
    ui->meshKeys->setText(hasCustomKeys ? tr("Keys*") : tr("Keys..."));
    ui->meshKeys->setToolTip(hasCustomKeys ?
        tr("Custom Meshcore decode keys configured. Click to edit.") :
        tr("Open Meshcore key manager."));

    applySettings();

    if (m_settings.m_meshcoreKeySpecList.isEmpty()) {
...
MeshcoreDemodSettings& MeshcoreDemodGUI::focusedSettings()
{
...
const MeshcoreDemodSettings& MeshcoreDemodGUI::focusedSettings() const
{
    if (m_focusedPipelineIndex > 0 && m_focusedPipelineIndex <= m_extraPipelineSettings.size()) {
        return m_extraPipelineSettings[m_focusedPipelineIndex - 1];
    }
...
int MeshcoreDemodGUI::pipelineCount() const
{
    return 1 + m_extraPipelineSettings.size();
}

void MeshcoreDemodGUI::updateConfControls()
{
...
void MeshcoreDemodGUI::loadFocusedSettingsToControls()
{
    const auto& s = focusedSettings();
    const int thisBW = MeshcoreDemodSettings::bandwidths[s.m_bandwidthIndex];
    ...
    int regionIndex = ui->meshRegion->findText(s.m_meshcoreRegionCode);
    if (regionIndex < 0) { regionIndex = ui->meshRegion->findText("EU_868"); }
    if (regionIndex < 0) { regionIndex = 0; }
    ui->meshRegion->setCurrentIndex(regionIndex);

    int presetIndex = ui->meshPreset->findText(s.m_meshcorePresetName);
    if (presetIndex < 0) { presetIndex = ui->meshPreset->findText("EU_NARROW"); }
    ...
    m_meshControlsUpdating = true;
    int channelIndex = ui->meshChannel->findData(s.m_meshcoreChannelIndex);
...
void MeshcoreDemodGUI::applyFocusedPipelineSettings(bool force)
{
    if (m_focusedPipelineIndex == 0) {
        applySettings(force);
    } else {
...
void MeshcoreDemodGUI::pushExtraPipelineSettingsToDemod()
{
    if (!m_doApplySettings) {
        return;
    }
    MeshcoreDemod::MsgSetExtraPipelineSettings* msg =
        MeshcoreDemod::MsgSetExtraPipelineSettings::create(m_extraPipelineSettings);
    m_meshcoreDemod->getInputMessageQueue()->push(msg);
}
...
void MeshcoreDemodGUI::on_conf_valueChanged(int value)
{
...
void MeshcoreDemodGUI::on_confAdd_clicked(bool checked)
{
    (void) checked;
...
void MeshcoreDemodGUI::on_confDel_clicked(bool checked)
{
    (void) checked;
...
int MeshcoreDemodGUI::findBandwidthIndex(int bandwidthHz) const
{
    int bestIndex = -1;
    int bestDelta = 1 << 30;
    ...
    {
        const int delta = std::abs(MeshcoreDemodSettings::bandwidths[i] - bandwidthHz);
        if (delta < bestDelta)
        {
            bestDelta = delta;
            bestIndex = i;
...
bool MeshcoreDemodGUI::retuneDeviceToFrequency(qint64 centerFrequencyHz)
{
...
bool MeshcoreDemodGUI::autoTuneDeviceSampleRateForBandwidth(int bandwidthHz, QString& summary, int* newBasebandSampleRateOut)
{
    summary.clear();
    ...
    const int deviceSetIndex = m_meshcoreDemod->getDeviceSetIndex();
...
void MeshcoreDemodGUI::applyMeshcoreProfileFromSelection()
{
    ...
    if (preset == "USER")
    {
        ...
        if (s.m_meshcoreRegionCode != region)
        {
            s.m_meshcoreRegionCode = region;
            selectionStateChanged = true;
        }

        if (s.m_meshcorePresetName != preset)
        {
            s.m_meshcorePresetName = preset;
            selectionStateChanged = true;
        }
        ...
        if (isPrimary && m_settings.m_meshcoreAutoSampleRate) {
    ...
    {
        qWarning() << "MeshcoreDemodGUI::applyMeshcoreProfileFromSelection:" << error;
        return;
    }
    ...
    if (s.m_meshcoreChannelIndex != meshChannel)
    {
        s.m_meshcoreChannelIndex = meshChannel;
        selectionStateChanged = true;
    }
    ...
    if (meshRadio.hasCenterFrequency)
    {
        if (isPrimary)
        {
            ...
            {
                qWarning() << "MeshcoreDemodGUI::applyMeshcoreProfileFromSelection: cannot retune device and device center frequency unknown";
            }
        }
        else
        {
    ...
    if (isPrimary)
    {
        if (m_settings.m_meshcoreAutoSampleRate) {
            sampleRateChanged = autoTuneDeviceSampleRateForBandwidth(thisBW, sampleRateSummary, &newBasebandSampleRate);
        } else {
    ...
    qInfo() << "MeshcoreDemodGUI::applyMeshcoreProfileFromSelection:"
...
void MeshcoreDemodGUI::setupMeshcoreAutoProfileControls()
{
    // Hide Region and Channel controls — region is implicit in each
    // preset's frequency, and group channels are managed via the keys
    // dialog rather than a numbered Channel selector.  Preset combo +
    // Apply button stay visible (see
    ...
    // Keep the still-applicable controls (meshKeys, meshAutoLock,
    // meshAutoSampleRate) — they govern the LoRa PHY auto-tune + the
    // operator's identity/contact/channel store, both of which apply to
    // MeshCore unchanged.
    ui->meshAutoSampleRate->setChecked(m_settings.m_meshcoreAutoSampleRate);

    QObject::connect(ui->meshKeys, &QPushButton::clicked, this, &MeshcoreDemodGUI::on_meshKeys_clicked);
    QObject::connect(ui->meshAutoLock, &QPushButton::clicked, this, &MeshcoreDemodGUI::on_meshAutoLock_clicked);
    QObject::connect(ui->meshAutoSampleRate, &QCheckBox::toggled, this, &MeshcoreDemodGUI::on_meshAutoSampleRate_toggled);
}

void MeshcoreDemodGUI::rebuildMeshcoreChannelOptions()
{
    const QString region = ui->meshRegion->currentText();
    const QString preset = ui->meshPreset->currentText();
    ...
    if (preset == "USER")
    {
        ...
        m_meshControlsUpdating = false;
        // Do NOT queue applyMeshcoreProfileFromSelection here: this function is
        ...
        // → rebuildMeshcoreChannelOptions → apply → …).
    ...
    ui->meshChannel->setToolTip(tr("Meshcore channel number (zero-based, shown with center frequency)"));
    ...
    qInfo() << "MeshcoreDemodGUI::rebuildMeshcoreChannelOptions:"
            << "region=" << region
            << "preset=" << preset
            << "channels=" << added;
}

void MeshcoreDemodGUI::onWidgetRolled(QWidget* widget, bool rollDown)
{
    (void) widget;
    (void) rollDown;
...
void MeshcoreDemodGUI::onMenuDialogCalled(const QPoint &p)
{
    if (m_contextMenuType == ContextMenuType::ContextMenuChannelSettings)
    {
        ...
        {
            dialog.setNumberOfStreams(m_meshcoreDemod->getNumberOfDeviceStreams());
...
MeshcoreDemodGUI::MeshcoreDemodGUI(PluginAPI* pluginAPI, DeviceUISet *deviceUISet, BasebandSampleSink *rxChannel, QWidget* parent) :
	ChannelGUI(parent),
	ui(new Ui::MeshcoreDemodGUI),
    ...
    m_tickCount(0)
{
    ...
    m_helpURL = "plugins/channelrx/demodmeshcore/readme.md";
    RollupContents *rollupContents = getRollupContents();
	ui->setupUi(rollupContents);
    setupMeshcoreAutoProfileControls();
    QObject::connect(ui->conf, QOverload<int>::of(&QDial::valueChanged), this, &MeshcoreDemodGUI::on_conf_valueChanged);
    QObject::connect(ui->confAdd, &QPushButton::clicked, this, &MeshcoreDemodGUI::on_confAdd_clicked);
    QObject::connect(ui->confDel, &QPushButton::clicked, this, &MeshcoreDemodGUI::on_confDel_clicked);
    updateConfControls();
    setupPipelineViews();
	...
	m_meshcoreDemod = (MeshcoreDemod*) rxChannel;
    m_spectrumVis = m_meshcoreDemod->getSpectrumVis();
	m_spectrumVis->setGLSpectrum(ui->glSpectrum);
    m_meshcoreDemod->setMessageQueueToGUI(getInputMessageQueue());

    connect(&MainCore::instance()->getMasterTimer(), SIGNAL(timeout()), this, SLOT(tick()));

    SpectrumSettings spectrumSettings = m_spectrumVis->getSettings();
    // Meshcore dechirp view defaults: keep lower pane active so replay-on-click
    ...
    ui->BW->setToolTip(tr("LoRa bandwidth selection. Meshcore presets auto-set this."));
    ...
    ui->preambleChirps->setToolTip(tr("LoRa preamble chirps. MeshCore: 32 for SF<9, 16 for SF>8 (SF-based auto-set on preset)."));
    ...
    resetLoRaStatus();
	applySettings(true);
    // On first creation, combo signals haven't fired yet. Apply selected Meshcore profile once.
...
MeshcoreDemodGUI::~MeshcoreDemodGUI()
{
	delete ui;
}

void MeshcoreDemodGUI::blockApplySettings(bool block)
{
    m_doApplySettings = !block;
}

void MeshcoreDemodGUI::applySettings(bool force)
{
	if (m_doApplySettings)
	{
        setTitleColor(m_channelMarker.getColor());
        MeshcoreDemod::MsgConfigureMeshcoreDemod* message = MeshcoreDemod::MsgConfigureMeshcoreDemod::create( m_settings, force);
        m_meshcoreDemod->getInputMessageQueue()->push(message);
	}
}

void MeshcoreDemodGUI::updateControlAvailabilityHints()
{
    const bool loRaMode = m_settings.m_codingScheme == MeshcoreDemodSettings::CodingLoRa;
    const bool explicitHeaderMode = loRaMode && MeshcoreDemodSettings::m_hasHeader;
    ...
    const bool isUserPreset = focusedSettings().m_meshcorePresetName.trimmed().compare("USER", Qt::CaseInsensitive) == 0;
    ...
    const bool headerControlsEnabled = !MeshcoreDemodSettings::m_hasHeader;
...
void MeshcoreDemodGUI::displaySettings()
{
    const auto& s = focusedSettings();
    // Primary pipeline bandwidth for channel marker and spectrum display.
    const int thisBWPrimary = MeshcoreDemodSettings::bandwidths[m_settings.m_bandwidthIndex];
    // Focused pipeline bandwidth for the BW slider and its label.
    const int thisBWFocused = MeshcoreDemodSettings::bandwidths[s.m_bandwidthIndex];
    ...
    const bool hasCustomKeys = !m_settings.m_meshcoreKeySpecList.trimmed().isEmpty();
    ui->meshKeys->setText(hasCustomKeys ? tr("Keys*") : tr("Keys..."));
    ui->meshKeys->setToolTip(hasCustomKeys ?
        tr("Custom Meshcore decode keys configured. Click to edit.") :
        tr("Open Meshcore key manager."));
    ...
    ui->meshRegion->setEnabled(s.m_meshcorePresetName != "USER");
    ui->BW->setEnabled(s.m_meshcorePresetName == "USER");
    ui->Spread->setEnabled(s.m_meshcorePresetName == "USER");
    ui->deBits->setEnabled(s.m_meshcorePresetName == "USER");
    ui->preambleChirps->setEnabled(s.m_meshcorePresetName == "USER");
...
void MeshcoreDemodGUI::displaySquelch()
{
...
void MeshcoreDemodGUI::displayLoRaStatus(int headerParityStatus, bool headerCRCStatus, int payloadParityStatus, bool payloadCRCStatus)
{
    if (MeshcoreDemodSettings::m_hasHeader && (headerParityStatus == (int) MeshcoreDemodSettings::ParityOK)) {
        ui->headerHammingStatus->setStyleSheet("QLabel { background-color : green; }");
    } else if (MeshcoreDemodSettings::m_hasHeader && (headerParityStatus == (int) MeshcoreDemodSettings::ParityError)) {
        ui->headerHammingStatus->setStyleSheet("QLabel { background-color : red; }");
    } else if (MeshcoreDemodSettings::m_hasHeader && (headerParityStatus == (int) MeshcoreDemodSettings::ParityCorrected)) {
    ...
    if (MeshcoreDemodSettings::m_hasHeader && headerCRCStatus) {
        ui->headerCRCStatus->setStyleSheet("QLabel { background-color : green; }");
    } else if (MeshcoreDemodSettings::m_hasHeader && !headerCRCStatus) {
...
void MeshcoreDemodGUI::resetLoRaStatus()
{
    ui->headerHammingStatus->setStyleSheet("QLabel { background:rgb(79,79,79); }");
    ui->headerCRCStatus->setStyleSheet("QLabel { background:rgb(79,79,79); }");
    ui->payloadFECStatus->setStyleSheet("QLabel { background:rgb(79,79,79); }");
...
void MeshcoreDemodGUI::setBandwidths()
{
    int maxBandwidth = m_basebandSampleRate/MeshcoreDemodSettings::oversampling;
    int maxIndex = 0;

    for (; (maxIndex < MeshcoreDemodSettings::nbBandwidths) && (MeshcoreDemodSettings::bandwidths[maxIndex] <= maxBandwidth); maxIndex++)
    {}

    if (maxIndex != 0)
    {
        qDebug("MeshcoreDemodGUI::setBandwidths: avl: %d max: %d", maxBandwidth, MeshcoreDemodSettings::bandwidths[maxIndex-1]);
        ui->BW->setMaximum(maxIndex - 1);
        int index = ui->BW->value();
        ui->BWText->setText(QString("%1 Hz").arg(MeshcoreDemodSettings::bandwidths[index]));
    }
}

void MeshcoreDemodGUI::setupPipelineViews()
{
    m_pipelineTabs = ui->pipelineTabs;
    ensurePipelineView(-1, "All");
}

MeshcoreDemodGUI::PipelineView& MeshcoreDemodGUI::ensurePipelineView(int pipelineId, const QString& pipelineName)
{
    ...
    QObject::connect(view.treeWidget, &QTreeWidget::itemSelectionChanged, this, &MeshcoreDemodGUI::onPipelineTreeSelectionChanged);
...
void MeshcoreDemodGUI::clearPipelineViews()
{
    setDechirpInspectionMode(false);
...
QString MeshcoreDemodGUI::buildPipelineMessageBaseKey(int pipelineId, uint32_t frameId, const QString& timestamp) const
{
    if (frameId != 0U) {
...
QString MeshcoreDemodGUI::allocatePipelineMessageKey(const QString& baseKey)
{
...
QString MeshcoreDemodGUI::resolvePipelineMessageKey(const QString& baseKey) const
{
...
void MeshcoreDemodGUI::consumePipelineMessageKey(const QString& baseKey, const QString& key)
{
...
void MeshcoreDemodGUI::rememberLoRaDechirpSnapshot(
...
void MeshcoreDemodGUI::setDechirpInspectionMode(bool enabled)
{
...
void MeshcoreDemodGUI::updateDechirpModeUI()
{
...
void MeshcoreDemodGUI::queueReplayForTree(QTreeWidget *treeWidget)
{
    m_replayPendingMessageKey.clear();
    m_replayPendingHasSelection = false;
...
void MeshcoreDemodGUI::processQueuedReplay()
{
    m_replaySelectionQueued = false;
...
void MeshcoreDemodGUI::hardResetDechirpDisplayBuffers()
{
    if (!ui || !ui->glSpectrum) {
        return;
    }
...
void MeshcoreDemodGUI::clearTreeMessageKeyReferences(const QString& messageKey)
{
    if (messageKey.trimmed().isEmpty()) {
        return;
    }
...
void MeshcoreDemodGUI::replayDechirpSnapshot(const DechirpSnapshot& snapshot)
{
...
void MeshcoreDemodGUI::onPipelineTreeSelectionChanged()
{
...
void MeshcoreDemodGUI::appendPipelineLogLine(int pipelineId, const QString& pipelineName, const QString& line)
{
...
void MeshcoreDemodGUI::appendPipelineStatusLine(int pipelineId, const QString& pipelineName, const QString& status)
{
    appendPipelineLogLine(pipelineId, pipelineName, QString(">%1").arg(status));
}

void MeshcoreDemodGUI::appendPipelineBytes(int pipelineId, const QString& pipelineName, const QByteArray& bytes)
{
    QStringList lines;
    QString line;
...
void MeshcoreDemodGUI::appendPipelineTreeFields(
    int pipelineId,
...
void MeshcoreDemodGUI::showLoRaMessage(const Message& message)
{
    const MeshcoreDemodMsg::MsgReportDecodeBytes& msg = (MeshcoreDemodMsg::MsgReportDecodeBytes&) message;
    ...
    if (msg.getEarlyEOM())
    {
        ...
        displayLoRaStatus(msg.getHeaderParityStatus(), msg.getHeaderCRCStatus(), (int) MeshcoreDemodSettings::ParityUndefined, true);
    ...
    // be selected for dechirp replay, even when higher-layer Meshcore parsing
...
void MeshcoreDemodGUI::showTextMessage(const Message& message)
{
    const MeshcoreDemodMsg::MsgReportDecodeString& msg = (MeshcoreDemodMsg::MsgReportDecodeString&) message;
    const int pipelineId = msg.getPipelineId();
    const QString pipelineName = msg.getPipelineName().trimmed().isEmpty()
        ? (pipelineId < 0 ? QString("Main") : QString("P%1").arg(pipelineId))
        : msg.getPipelineName();
...
void MeshcoreDemodGUI::displayText(const QString& text)
{
    appendPipelineLogLine(-1, "All", QString("TXT|%1").arg(text));
}

void MeshcoreDemodGUI::displayBytes(const QByteArray& bytes)
{
    appendPipelineBytes(-1, "All", bytes);
}

void MeshcoreDemodGUI::displayStatus(const QString& status)
{
    appendPipelineStatusLine(-1, "All", status);
    qInfo() << "MeshcoreDemodGUI::displayStatus:" << status;
}

QString MeshcoreDemodGUI::getParityStr(int parityStatus)
{
    if (parityStatus == (int) MeshcoreDemodSettings::ParityError) {
        return "err";
    } else if (parityStatus == (int) MeshcoreDemodSettings::ParityCorrected) {
        return "fix";
    } else if (parityStatus == (int) MeshcoreDemodSettings::ParityOK) {
        return "ok";
    } else {
        return "n/a";
    }
...
void MeshcoreDemodGUI::tick()
{
    ...
    if (m_deviceUISet && m_deviceUISet->m_deviceAPI)
    {
        ...
        if (isRemoteTcpInput)
        {
            ...
            if (running && !m_remoteTcpLastRunningState)
            {
                ...
                m_remoteTcpReconnectAutoApplyWaitTicks = 0;
                qInfo() << "MeshcoreDemodGUI::tick: RemoteTCP input running - waiting for first DSP notification to reapply Meshcore profile";
            }
            ...
            if (m_remoteTcpReconnectAutoApplyPending && running)
            {
                ...
                if (m_remoteTcpReconnectAutoApplyWaitTicks >= 20)
                {
                    m_remoteTcpReconnectAutoApplyPending = false;
                    m_remoteTcpReconnectAutoApplyWaitTicks = 0;
                    qInfo() << "MeshcoreDemodGUI::tick: RemoteTCP reconnect fallback timeout - reapplying Meshcore profile";
                    QMetaObject::invokeMethod(this, &MeshcoreDemodGUI::applyMeshcoreProfileFromSelection, Qt::QueuedConnection);
                }
            }
    ...
    else
    {
        ...
        ui->nText->setText(tr("%1").arg(CalcDb::dbPower(m_meshcoreDemod->getCurrentNoiseLevel()), 0, 'f', 1));
        ui->channelPower->setText(tr("%1 dB").arg(CalcDb::dbPower(m_meshcoreDemod->getTotalPower()), 0, 'f', 1));

        if (m_meshcoreDemod->getDemodActive()) {
...
void MeshcoreDemodGUI::makeUIConnections()
{
    QObject::connect(ui->deltaFrequency, &ValueDialZ::changed, this, &MeshcoreDemodGUI::on_deltaFrequency_changed);
    QObject::connect(ui->BW, &QSlider::valueChanged, this, &MeshcoreDemodGUI::on_BW_valueChanged);
    QObject::connect(ui->Spread, &QSlider::valueChanged, this, &MeshcoreDemodGUI::on_Spread_valueChanged);
    QObject::connect(ui->deBits, &QSlider::valueChanged, this, &MeshcoreDemodGUI::on_deBits_valueChanged);
    QObject::connect(ui->preambleChirps, &QSlider::valueChanged, this, &MeshcoreDemodGUI::on_preambleChirps_valueChanged);
    QObject::connect(ui->mute, &QToolButton::toggled, this, &MeshcoreDemodGUI::on_mute_toggled);
    ...
    QObject::connect(ui->messageLength, &QDial::valueChanged, this, &MeshcoreDemodGUI::on_messageLength_valueChanged);
...
#else
    QObject::connect(ui->udpSend, &QCheckBox::stateChanged, this, &MeshcoreDemodGUI::on_udpSend_stateChanged);
    QObject::connect(ui->udpSendJson, &QCheckBox::stateChanged, this, &MeshcoreDemodGUI::on_udpSendJson_stateChanged);
#endif
    QObject::connect(ui->udpAddress, &QLineEdit::editingFinished, this, &MeshcoreDemodGUI::on_udpAddress_editingFinished);
    QObject::connect(ui->udpPort, &QLineEdit::editingFinished, this, &MeshcoreDemodGUI::on_udpPort_editingFinished);
#if QT_VERSION >= QT_VERSION_CHECK(6, 9, 0)
...
#else
    QObject::connect(ui->invertRamps, &QCheckBox::stateChanged, this, &MeshcoreDemodGUI::on_invertRamps_stateChanged);
#endif
}

void MeshcoreDemodGUI::updateAbsoluteCenterFrequency()
{
