...
The Geometry rollup records the radar transmitter latitude, longitude, beam azimuth, elevation, horizontal coverage, and vertical half-power beamwidth (HPBW), plus the receive antenna azimuth, elevation, and HPBW. Receive pointing can be entered manually or synchronized once per second from a selected GS232Controller feature. Selecting no rotator restores manual receive azimuth and elevation entry. The current receive pointing and beamwidth are included in exported RMOB reports.

The map button sends translucent transmitter and receiver HPBW volumes to open Map features, together with their geometric intersection. The orange TX and cyan RX solids are capped at the selected maximum altitude, which defaults to 1000 km. Their brighter green overlap is the region inside both antenna HPBW envelopes; it is a geometric coverage volume rather than a received-power or radar-sensitivity prediction. The 2D map displays corresponding footprints. The GRAVES defaults use approximately 180 degrees of southern azimuthal coverage and 25 degrees of vertical HPBW. A zero horizontal coverage or HPBW leaves that antenna pattern unspecified and suppresses its volume.
