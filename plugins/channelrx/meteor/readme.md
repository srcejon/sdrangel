...
Rejected smooth Doppler sweeps are listed in the Satellites tab. Alongside signed center-Doppler speed, the table reports maximum absolute equivalent radial speed from the robust fitted sweep endpoints and average Doppler drift rate. An RF-only classification combines endpoint speed, drift rate, total frequency excursion, and track duration into Aircraft, Satellite, or Uncertain, with a heuristic satellite score. The score is not a calibrated probability.

When an ADS-B Demod channel is open, fresh aircraft position, altitude, ground speed, track or heading, and vertical-rate reports are converted to source-neutral moving-target states. The matcher extrapolates each state through the sweep interval, predicts the bistatic Doppler at the robust start and end times from the configured transmitter and receiver positions, and scores the observed center and drift residuals.

Satellite matching uses the same prediction and scoring path. A plugin-local worker thread downloads the CelesTrak active OMM CSV catalog, caches it as `meteor/celestrak-active.csv` under the application data directory, and refreshes it daily. The CSV adapter preserves six-digit NORAD catalog IDs while converting the orbital fields for the bundled SGP4 propagator. SGP4 propagation, horizon and configured TX/RX beam filtering, and matching all run on that worker. State snapshots are shared by detections in the same five-second interval to keep accelerated file replay responsive. Live sources use the detection's system UTC; FileInput and SigMF FileInput use the recording time from the device report, corrected for detection age and accelerated playback. The best ADS-B and TLE results are ranked together; a row is identified as `Aircraft (ADS-B)` or `Satellite (TLE)` only when its score is at least 60 and leads every other candidate by at least 8 points. Otherwise the RF-only class remains and close alternatives are shown as ambiguous. TLE matches are unavailable in builds where the SGP4 library was not found.
...
Smooth, sustained Doppler sweeps and tracks beyond the configured meteor drift limit are excluded from meteor counts and reported separately as satellite candidates. They appear in the Satellites table and use cyan waterfall boxes and labels. The report includes time, duration, center frequency, span, and drift measured by the same spectral tracker. Aircraft and other moving or swept carriers can have the same RF shape, so these rows are candidates rather than confirmed satellite identifications. A short 4- or 5-frame sweep can survive the meteor sweep gate only when it is spectrally coherent, locally strong, and compact in time, which retains brief head-echo-like meteor signatures without accepting sustained moving carriers. The detector remains primarily a trail-echo counter.
