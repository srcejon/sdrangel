...
QString serializeSpectrumOverlays(const QList<CameraSettings::SpectrumOverlay>& overlays)
{
    ...
    for (const CameraSettings::SpectrumOverlay& overlay : overlays)
    {
        ...
        object.insert(QStringLiteral("source"), overlay.m_source);
...
QList<CameraSettings::SpectrumOverlay> deserializeSpectrumOverlays(const QString& json)
{
    ...
    for (const QJsonValue& value : array)
    {
        ...
        overlay.m_source = object.value(QStringLiteral("source")).toString();
        if (overlay.m_source.isEmpty()) {
            overlay.m_source = object.value(QStringLiteral("device")).toString();
        }
...
bool CameraSettings::deserialize(const QByteArray& data)
{
    ...
    if (d.getVersion() == 1)
    {
        ...
        if (m_spectrumOverlays.isEmpty() && m_overlaySpectrum && !m_spectrumDevice.isEmpty())
        {
            ...
            overlay.m_source = m_spectrumDevice;
        ...
        {
            m_spectrumDevice = firstEnabledSpectrumOverlay->m_source;
        ...
        m_yoloDnnTarget = qBound(CPU, m_yoloDnnTarget, LiteRT_GPU);
...
void CameraSettings::applySettings(const QStringList& settingsKeys, const CameraSettings& settings)
{
    ...
    if (settingsKeys.contains("cloudEdgeMarginPercent")) {
        m_cloudEdgeMarginPercent = qBound(m_minCloudEdgeMargin, settings.m_cloudEdgeMarginPercent, m_maxCloudEdgeMargin);
    }
    ...
    if (settingsKeys.contains("spectrumOverlays")) {
        ...
        {
            m_spectrumDevice = firstEnabled->m_source;
    ...
    if (settingsKeys.contains("yoloDnnTarget")) {
        m_yoloDnnTarget = qBound(CPU, settings.m_yoloDnnTarget, LiteRT_GPU);
    }
...
QString CameraSettings::getDebugString(const QStringList& settingsKeys, bool force) const
{
    ...
    if (settingsKeys.contains("cloudEdgeMarginPercent") || force) {
        ostr << " m_cloudEdgeMarginPercent: " << m_cloudEdgeMarginPercent;
    }
