...
A fully clear sky is not required: when the frame as a whole is not verified clear, the regions that are - sky around visible predicted stars at night (star sensing must be enabled), confidently blue sky by day - still accumulate into the reference patchwork-style. Each pixel joins the comparison once it has been confirmed clear on two separate occasions, so at sites that never get a wholly clear night the reference assembles itself over time from whatever gaps appear; a later verified-clear frame fills the remaining gaps outright. The viewer's Filled column shows how much of each slot has been confirmed.
